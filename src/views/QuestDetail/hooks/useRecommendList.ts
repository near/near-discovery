...
export default function useRecommendList(campaign_id: string, page_size: number) {
    ...
    async (_page: number) => {
      ...
      try {
        const result = await get(
          `${QUEST_PATH}/api/quest/recommend_list?campaign_id=${campaign_id}&page=${_page}&page_size=${page_size}`,
        );
