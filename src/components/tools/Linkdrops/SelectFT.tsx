...
const SelectFT = ({tokens,setToken}:{tokens:any,setToken:(token:string)=>void}) => {
  ...
  return (
    ...
    <SelectContainer>
      ...
      {isOpen && (
        <OptionsList>
              <Option
              key={nearCoin.contract_id}
              onClick={() => {
                setSelectedCurrency(nearCoin);
                setIsOpen(false);
                setToken(nearCoin.contract_id);
              }}
            >
              <CurrencyInfo>
                <Image src={nearCoin.icon} alt={nearCoin.symbol} width={20} height={20} style={{marginRight:"8px"}}/>
                <span>{nearCoin.contract_id} ({nearCoin.symbol})</span>
              </CurrencyInfo>
            </Option>
          {tokens.map((token) => (
            <Option
