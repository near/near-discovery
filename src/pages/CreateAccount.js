...
import { createAccount, getCorrectAccessKey } from '../utils/biometricsAuthUtils';
...
const CreateAccount = () => {
      ...
      const response = await fetch('https://rpc.testnet.near.org', {
        ...
        body: JSON.stringify({
          ...
          "params": {
            ...
            "finality": "final",
            "account_id": `${formValues?.accountId}.testnet`
          }
        })
      });
  ...
  const onSubmit = handleSubmit(async (data) => {
    const name = `${data.accountId}.testnet`;
      ...
      <FormContainer onSubmit={onSubmit}>
        ...
        <InputContainer>
          ...
          <input
            ...
            statusMessage={isAccountAvailable === null ? 'Use a suggested ID or customize your own.' : !!isAccountAvailable ? `${formValues?.accountId}.testnet is available!` : `${formValues?.accountId}.testnet is taken, try something else.`}
          />
          <p className="subText">
            {isAccountAvailable === null ? 'Use a suggested ID or customize your own.' : !!isAccountAvailable ? `${formValues?.accountId}.testnet is available!` : `${formValues?.accountId}.testnet is taken, try something else.`}
          </p>
